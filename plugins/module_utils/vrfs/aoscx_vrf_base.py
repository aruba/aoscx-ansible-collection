...
from ansible_collections.arubanetworks.aoscx.plugins.module_utils.vrfs.aoscx_vrf_entry import VRF_Entry
...
class VRF_Base:
    ...
    def delete_vrf(self, vrf_name):
        ...
        if vrf_name in self.vrfs.keys():
            error = (
                "VRF {vrf_name} is attached to {int}. Interface must be deleted "
                "and "
            ...
            raise Exception(
                "VRF {name} does not exist".format(
                    name=vrf_name))
