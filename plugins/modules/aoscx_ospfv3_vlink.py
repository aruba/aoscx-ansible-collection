...
DOCUMENTATION = """
---
module: aoscx_ospfv3_vlink
version_added: "4.1.0"
short_description: Create or Delete OSPFv3 configuration on AOS-CX
description: >
  This modules provides configuration management of OSPFv3 Virtual Links on
  AOS-CX devices.
...
options:
  ...
  vrf:
    description: >
      The VRF the OSPFv3 Area will belong to once created. If the OSPFv3 Area
      is created and the user wants to change its VRF, the user must first
      delete the OSPFv3 Area, and then recreate it in the desired VRF.
    type: str
    required: true
  ospf_id:
  ...
  other_config:
    ...
    suboptions:
      ...
      virtual_ifindex:
        description: >
          The virtual link index assigned to the OSPFv3 virtual interface.
        type: int
        required: true
      retransmit_interval:
...
def main():
    ...
    if "ospf_auth_type" in params and params["ospf_auth_type"]:
    ...
    ospf_router = Ospfv3Router(session, ospf_id, vrf)
    try:
        ospf_router.get()
    except Exception:
        ansible_module.fail_json(
            msg="Could not find OSPFv3 Router, make sure it exists"
        )
    ...
    try:
        ...
        ansible_module.fail_json(
            msg="Could not find OSPFv3 Area, make sure it exists"
        )
