...
DOCUMENTATION = """
---
module: aoscx_ospfv3_router
version_added: "4.1.0"
short_description: Create, Update or Delete OSPFv3 configuration on AOS-CX
description: >
  This modules provides configuration management of OSPFv3 Routers on AOS-CX
  devices.
...
options:
  ...
  vrf:
    description: >
      The VRF the OSPFv3 Router will belong to once created. If none provided,
      it will be in the Default VRF. If the OSPFv3 Router is created and the
      user wants to change its VRF, the user must first delete the OSPFv3
...
def main():
    ...
    ospf_router = Ospfv3Router(session, instance_tag, vrf, **params)
    try:
        ospf_router.get()
        exists = True
    except Exception:
